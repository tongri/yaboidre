...
def start_bot():
    ...
    def handler_start(message):
        ...
        bot.send_message(chat_id,
                         '🔪Добро пожаловать {}, Твой ID - {} 🔪                               🚘 RDS TEAM - @drill_scm_bot 🚘'.format(message.from_user.first_name,
                                                                    chat_id,),
    ...
    def handler_admin(message):
        ...
        if chat_id == settings.admin_id:
            bot.send_message(chat_id, 'Салам, брат', reply_markup=menu.admin_menu)
    ...
    def handler_call(call):
        ...
        # Main menu
        if call.data == 'admin_info_settings':
            msg = bot.send_message(
                chat_id=chat_id,
                text=f'Текущая информация \n{func.info()}\n\nВведите значения в виде юзеров:кол-во покупок:доход (4342:142:39898)'
            )
            bot.clear_step_handler_by_chat_id(chat_id)
            bot.register_next_step_handler(msg, admin_info_settings)
    ...
    def admin_info_settings(message):
        try:
            users = message.text.split(':')[0]
            sales = message.text.split(':')[1]
            sum = message.text.split(':')[2]

            func.admin_info_settings(users, sales, sum)
