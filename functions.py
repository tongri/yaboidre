...
def info():
    ...
    msg = settings.info2.format(
        cursor.execute(f'SELECT * FROM info WHERE name = "users"').fetchone()[1],
        cursor.execute(f'SELECT * FROM info WHERE name = "sales"').fetchone()[1],
        cursor.execute(f'SELECT * FROM info WHERE name = "sum"').fetchone()[1],
    )

    return msg


def admin_info_settings(users, sales, sum):
    conn = sqlite3.connect("base_ts.sqlite")
    cursor = conn.cursor()

    cursor.execute(f'UPDATE info SET value = {users} WHERE name = "users"')
    conn.commit()

    cursor.execute(f'UPDATE info SET value = {sales} WHERE name = "sales"')
    conn.commit()

    cursor.execute(f'UPDATE info SET value = {sum} WHERE name = "sum"')
    conn.commit()